document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateKPIs() {
        ...
        const randomOrders = Math.floor(Math.random() * 500) + 50;
        const randomUsers = Math.floor(Math.random() * 1000) + 200;
        const randomConversion = (Math.random() * 5 + 1).toFixed(1);

        document.getElementById('kpi-revenue').innerText = `$${randomRevenue.toLocaleString()}`;
        document.getElementById('kpi-orders').innerText = randomOrders;
        document.getElementById('kpi-users').innerText = randomUsers;
        document.getElementById('kpi-conversion').innerText = `${randomConversion}%`;
    }

    // Update every 5 seconds
    setInterval(updateKPIs, 5000);
    updateKPIs(); // Initial call
